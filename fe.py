...
def code_agent(dataset_summary: str, suggestions: str,
    ...
    system_prompt = (
        ...
        "  7. use pd.qcut for binning numeric features into quantiles, not pd.cut.\n"
    )
...
def identify_target_column(df: pd.DataFrame, csv_path: str) -> str | None:
    ...
    EXACT_TARGET_NAMES = {
        ...
        "status", "price", "salary", "revenue", "sales", "profit",
        "score", "rating", "grade", "risk", "hazard", "event",
    ...
    PARTIAL_TARGET_KEYWORDS = [
        "target", "label", "class", "churn", "fraud", "default",
        "outcome", "result", "status", "price", "salary", "revenue",
        "sales", "profit", "score", "rating", "grade", "risk",
        "predict", "response", "dependent", "output", "flag",
        "indicator", "event", "survived", "death", "income",
    ...
    for col in columns:
        ...
        # Last column (ML datasets often put target last)
        if col == columns[-1]:
            score += 5
