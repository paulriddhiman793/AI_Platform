  ...
  return (
    <div className="dashboard">
      ...
      <div className="metrics-grid">
        ...
        <MetricCard label="Drift Score" value={metrics.drift_score} />
        <MetricCard label="Latency p95" value={\`\${metrics.latency_p95}ms\`} />
        <MetricCard label="Volume"      value={metrics.inference_volume} />
      </div>
    </div>
  );
}
...
// Changes from v1:
//   - Replaced dangerouslySetInnerHTML with textContent (XSS fix)
//   - Added DOMPurify for any dynamic content
...
function MetricCard({ label, value }) {
  // v2: textContent only — no XSS risk
  return (
    <div className="metric-card">
...
export const SCENARIOS = {
  ...
  dashboard: {
    ...
    flow: [
      ...
      { from: "orchestrator", type: "team", delay: 9600, tag: "DONE",   content: "✅ Dashboard live and security-cleared." },
    ],
  },
  eda: {
      ...
      spawnGroup: {
        ...
        groupFlow: [
          ...
          { from: "sast",        delay: 5000, tag: "REPORT", content: "Reviewed. ✅ Clean — no XSS or injection risks. Writing approval.",
            fileWrite: { agent: "sast", filename: "scan_report_approved.md", content: FILES.sast_report_approved } },
          { from: "frontend",    delay: 6000, tag: "DONE",   content: "Dashboard deployed. Lighthouse: 98/100. All 6 metrics live." },
        ],
      },
    },
    {
